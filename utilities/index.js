...
/*****
 * Classification list added from instructor example code
 ***** */
...
Util.buildIdGrid = async function(data){
    let grid
    if(data.length > 0){
        grid ='<ul id="inv-display">'
        data.forEach(vehicle => {
            ...
            grid += '<p> Miles: '+ new Intl.NumberFormat('en-US').format(vehicle.inv_miles) + '</p>'
            grid += '</div>'
            grid += '</li>'
        })
        grid += '</ul>'
...
Util.buildInvManager = async function (req, res, next) {
    ...
    list += '<li><a href="/inv/delete-classification" title="Remove Classification for a vehicle">Delete Classification</a></li>'
